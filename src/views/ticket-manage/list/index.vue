...
import { buildTicketStatCards, getActiveTicketStatKey } from '@/constants/ticket-statistics';
import type { TicketStatCardItem } from '@/constants/ticket-statistics';
import {
  fetchAutoAssignTicket,
  fetchDeleteTicket,
  fetchGetTicketList,
  fetchGetTicketStatisticsSummary
} from '@/service/api';
...
function getInitSearchParams(): Api.Ticket.TicketSearchParams {
  ...
  return {
    ...
    isOverdue: query.isOverdue === 'true' || undefined
  };
}

const statCards = ref<TicketStatCardItem[]>(buildTicketStatCards(null));

async function loadStatistics() {
  const { data, error } = await fetchGetTicketStatisticsSummary();

  if (!error) {
    statCards.value = buildTicketStatCards(data);
  }
}
...
function handleStatCardClick(card: TicketStatCardItem) {
  searchParams.value.status = card.filter.status;
  searchParams.value.isOverdue = card.filter.isOverdue;
  getDataByPage();
}
  ...
  <div class="flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <div class="flex flex-wrap gap-8px">
      <div
        ...
        :key="card.key"
        class="min-w-110px flex-1 cursor-pointer border border-#d9d9d9 rd-6px px-12px py-8px transition-colors"
        :class="activeStatKey === card.key ? 'border-primary bg-primary-light-9' : 'hover:border-primary'"
        @click="handleStatCardClick(card)"
      >
        <div class="text-placeholder text-12px">{{ card.label }}</div>
        <div class="mt-2px text-20px font-medium">{{ card.value }}</div>
      </div>
    </div>
    <ElCard class="card-wrapper sm:flex-1-hidden">
      ...
      <TicketCreateDrawer v-model:visible="createVisible" @submitted="handleCreated" />
    </ElCard>
  </div>
</template>
