...
defineOptions({ name: 'WorkGroupSearch' });
...
const model = defineModel<Api.WorkGroup.WorkGroupSearchParams>('model', { required: true });
...
<template>
  <ElCard class="card-wrapper">
    <ElCollapse>
      <ElCollapseItem :title="$t('common.search')" name="work-group-search">
        <ElForm :model="model" label-position="right" :label-width="90">
          <ElRow :gutter="24">
            <ElCol :lg="8" :md="12" :sm="12">
              <ElFormItem :label="$t('page.manage.workGroup.keyword')" prop="keyword">
                <ElInput v-model="model.keyword" :placeholder="$t('page.manage.workGroup.form.keyword')" />
              </ElFormItem>
            </ElCol>
            <ElCol :lg="8" :md="12" :sm="12">
