...
import { fetchGetUserOptions } from '@/service/api';
import { $t } from '@/locales';

defineOptions({ name: 'TodoManageSearch' });
...
const model = defineModel<Api.Todo.TodoAdminSearchParams>('model', { required: true });
...
const assigneeOptions = shallowRef<CommonType.Option<number>[]>([]);

onMounted(async () => {
  const { data, error } = await fetchGetUserOptions();
...
<template>
  <ElCard class="card-wrapper">
    <ElCollapse>
      <ElCollapseItem :title="$t('common.search')" name="todo-manage-search">
        <ElForm :model="model" label-position="right" :label-width="90">
          <ElRow :gutter="24">
            ...
            <ElCol :lg="6" :md="8" :sm="12">
              <ElFormItem :label="$t('page.todo.manage.assigneeId')" prop="assigneeId">
                <ElSelect
                  v-model="model.assigneeId"
                  clearable
                  filterable
                  :placeholder="$t('page.todo.manage.form.assignee')"
                >
                  <ElOption v-for="{ label, value } in assigneeOptions" :key="value" :label="label" :value="value" />
                </ElSelect>
              </ElFormItem>
            </ElCol>
            <ElCol :lg="6" :md="8" :sm="12">
