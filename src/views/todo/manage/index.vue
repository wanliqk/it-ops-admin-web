...
import { fetchCancelTodo, fetchGetTodoList } from '@/service/api';
...
import TodoManageSearch from './modules/todo-manage-search.vue';
...
function getInitSearchParams(): Api.Todo.TodoAdminSearchParams {
  return {
    current: 1,
    size: 10,
    keyword: undefined,
...
const { columns, data, getData, getDataByPage, loading, mobilePagination } = useUIPaginatedTable({
  ...
  api: () => fetchGetTodoList(searchParams.value),
  ...
  columns: () => [
    { prop: 'todoNo', label: $t('page.todo.todoNo'), width: 170 },
    ...
    { prop: 'completedAt', label: $t('page.todo.completedAt'), width: 160, formatter: row => row.completedAt ?? '-' },
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <TodoManageSearch v-model:model="searchParams" @reset="resetSearchParams" @search="getDataByPage" />
    <ElCard class="card-wrapper sm:flex-1-hidden">
      <template #header>
        <div class="flex items-center justify-between">
          <p>{{ $t('page.todo.manage.title') }}</p>
